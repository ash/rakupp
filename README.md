...
**Status:** measured per individual test, **~87% of Roast passes** — 186,778 of
~214,528 tests the suite declares, counting the tests in files that abort before
running (their `plan N` is read from source). On the stricter all-or-nothing bar,
**528 / 1,462 files fully pass (~36%)** — a file counts only if *every* assertion
