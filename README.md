...
**Status:** measured per individual test, **~89% of Roast passes** — 193,223 of
~215,806 tests the suite declares, counting the tests in files that abort before
running (their `plan N` is read from source). On the stricter all-or-nothing bar,
**573 / 1,462 files fully pass (~39%)** — a file counts only if *every* assertion
