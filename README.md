...
**Status:** measured per individual test, **~88% of Roast passes** — 189,102 of
~214,384 tests the suite declares, counting the tests in files that abort before
running (their `plan N` is read from source). On the stricter all-or-nothing bar,
**558 / 1,462 files fully pass (~38%)** — a file counts only if *every* assertion
...
- **[presentation/](presentation/)** — a self-contained slide deck introducing Raku++ and its ecosystem (open [`presentation/index.html`](presentation/index.html) in a browser). The quickest visual tour.
