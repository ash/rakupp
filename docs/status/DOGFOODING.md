...
| [`showcase/rakus/rakus.raku`](../../showcase/rakus) | The static HTTP server the built site is previewed on — raw `IO::Socket::INET`, a thread per connection, correct `application/wasm` so the browser can stream-compile the engine. Published to the ecosystem as `App::Rakus`. | `rakupp showcase/rakus/rakus.raku 8973 www` |

So a page about Raku++ is generated by a Raku program run by Raku++, checked
by running its examples under Raku++, served by a Raku web server run by
Raku++ — and then executes those examples in the reader's browser on the same
