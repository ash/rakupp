...
| 2 | Assertions of **tests that ran** | 198,628 / 205,087 (~97%) | numerator ÷ assertions the files actually emitted |
| 3 | Assertions of **tests planned** | 198,628 / 215,639 (~91%) | ÷ the plan `N` of every file that emitted a plan (so tests lost to a mid-file abort count against us) |
| 4 | Assertions of **all declared tests** | 198,628 / 218,626 (**~90%**) | ÷ every test any file declares — including files that abort before emitting TAP, whose `plan N` is read from source |
...
source and counts all N as failing. That is why 4's denominator (218,626) is ~2.8k larger
than 3's (215,639) — those 2,819 tests live in 85 no-TAP files (parse errors
...
denominator.** Our current run recovers **218,626** declared tests. (This number
...
So our same 198,628 passes read two ways:

- **~90%** against *our* denominator (198,628 / 218,626) — *"of the tests we can
