...
|---|---|---|---|
| 1 | **Files fully passing** | 669 / 1,464 (**~46%**) | a file counts only if *every* planned assertion passes (or it legitimately `plan skip-all`s) |
| 2 | Assertions of **tests that ran** | 200,432 / 206,564 (~97%) | numerator ÷ assertions the files actually emitted |
| 3 | Assertions of **tests planned** | 200,432 / 216,570 (~92%) | ÷ the plan `N` of every file that emitted a plan (so tests lost to a mid-file abort count against us) |
| 4 | Assertions of **all declared tests** | 200,432 / 219,558 (**~91%**) | ÷ every test any file declares — including files that abort before emitting TAP, whose `plan N` is read from source |
...
source and counts all N as failing. That is why 4's denominator (219,294) is ~3.1k larger
than 3's (216,223) — those 3,071 tests live in 77 no-TAP files (parse errors
...
denominator.** Our current run recovers **219,294** declared tests. (This number
...
So our same 200,432 passes read two ways:

- **~91%** against *our* denominator (200,432 / 219,558) — *"of the tests we can
