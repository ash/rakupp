...
**Headline: ~90% of all declared Roast tests pass** (198,660 / 218,660); on the
stricter file bar, ~43% of files fully pass (632 / 1,464). The per-file breakdown
...
|---|---:|---:|
| **Fully passing** | **632** | **43%** |
| Partially passing | 695 | 47% |
| No TAP output | 124 | 8% |
| Timeouts | 13 | 0.9% |
...
**198,660 of ~218,660 declared tests — 90.9%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 198,660 / 205,106 (~97%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 198,660 / 215,673 (~92%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 198,660 / 218,660 (90.9%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |

The 90% is the per-test analog of the ~43% file coverage. Three notes on scope:

1. **~3.0k of the denominator comes from no-TAP files** (77 of them, read from
...
| S12 | Objects & classes | 26 | 63 | 0 | 12 | 1248/1499 | 83% |
...
| S17 | Concurrency (supply/promise/async) | 42 | 42 | 6 | 9 | 942/1092 | 86% |
...
| S32 | Standard types (str/list/num/…) | 120 | 125 | 2 | 16 | 41817/44407 | 94% |
...
_Snapshot 2026-08-21: 632 / 1,464 files fully passing (~43% coverage); 695
partial, 124 no-TAP, 13 timeout (the scheduler/io timing files flap between
pass and timeout under runner load; four runs on one machine that day gave
631 / 632 / 631 / 632 files, and the figures quoted here are the run whose
fully-passing list contains every file the others passed). This snapshot adds
...
pressure, and at program end). Reached-assertion
pass rate 198,660 / 205,106 (see caveat above — not a coverage figure).
