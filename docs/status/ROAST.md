...
**Headline: ~90% of all declared Roast tests pass** (197,080 / 218,613); on the
...
**197,080 of ~218,613 declared tests — 90%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 197,080 / 203,502 (~97%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 197,080 / 215,794 (~91%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 197,080 / 218,613 (90%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
_Snapshot: 594 / 1,462 files fully passing (~41% coverage); 721 partial,
133 no-TAP, 14 timeout (the scheduler/io timing files flap between pass and
timeout under runner load; four runs on one machine gave 197,082 / 197,056 /
197,098 / 197,080 assertions across 595 / 593 / 594 / 594 files, and the
figures quoted here are the repeating profile rather than the best run — the
fourth run was taken precisely because the first three had no repeat).
Reached-assertion pass rate 197,080 / 203,502 (see caveat above — not a
coverage figure). S05-substitution is a fully-passing subchapter (67222.t,
match.t, subst.t)._
