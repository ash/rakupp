...
| 2026-09-01 | **v3.24.0** | **what other people's code asked for** — the first release after the consolidation arc, and almost none of it was chosen here: eight GitHub issues, six third-party distributions that would not install, and one outside report that two dists ran ~40% slower than Rakudo. The ecosystem was **measured again rather than carried forward** — **746 of 2,526** distributions passing their own suites, up from 637, decomposing as 125 conversions, 18 reproducible regressions and 2 new dists ([ECOSWEEP](../dev/findings/ECOSWEEP-2026-08.md)). [#47] drove DISPATCH-PERF-PLAN to its end: five OO kernels added to `perf-guard` **first**, because the gate covered sub calls at ~2x Rakudo and could not see method calls at 5.8x at all; then `invokeMethod` stopped allocating a frame that sub calls already took from a pool, the attribute arm moved above ~100 string compares, and one thread-local resolution replaced eighty-four. Measured against v3.23.0's code **rebuilt on the same machine the same day**: loopsum **-11.9%**, fib **-11.5%**, strpass **-9.4%**, subcall **-9.1%**, asg **-7.5%**. Gate 1 then earned its keep: three files regressed while the headline count moved 643 → 640, inside the flap band — **two real bugs**, `my %`/`my &` dying in any block (the parser's own `%!anon` read as a private attribute by the hoisted arm) and every **DateTime** handed to a timer landing ten seconds in the past (`sleep-until`, `Promise.at`, `Scheduler.cue(:at)`, `$*INIT-INSTANT` — only two of the four had any test anywhere). Both fixed, both covered. Roast **199,846 / 219,374 (91.1%)**, **646 / 1,464 files** (band 645/646/646, union diff empty in the regressed direction, three gained); regression suite **609**; battery 50/59; documentation examples **953**; ecosystem 746. `strscan` is **+6.5%** and recorded rather than fixed: it is the kernel that walks the deepest part of the built-in method ladder 400,000 times, and the named fix is method-name slot indices — a project, not a patch. |
| 2026-09-03 | **v3.25.0** | **the roots under the top hundred** — v3.24.0 followed what other people's code asked for; this release reads the same source of work at ecosystem scale: the hundred most depended-on dists **and the 69 underneath them** ([ECOSYSTEM-TOP100](../dev/ecosystem/ECOSYSTEM-TOP100.md)), measured as one battery from an empty store and worked by *cause* — one engine fault usually sits under several rows. **62 → 67 of 100** passing their own suites, **58 → 60** end to end, **95 → 103 of 169** across the closure; twelve rows converted, none lost. The refresh earned its keep on the rows that had *broken* since the pinned sweep: URI (`Nil` assigned through an accessor was type-checked instead of resetting the container), Config (the installer ranked a dist that merely listed the module above the one named for it), Log::Async (one process-wide "current line" slot written by two threads — 7 in 20 flaky, 0 in 30 after). The engine faults behind the rows, each fixed by cause and named with the rule it restores: `<name=$pattern>` is a call not a group (#54), a zen slice decontainerises (#55, a gap since v0.1.0), Whatever-currying is syntactic, `Nil ~~ UInt` is False, `nextcallee`, `.clone` diffed over ~130 cases, NativeCall's `void` and the seven gaps behind it (#57), a kinded value crosses the extension ABI as `RK_OTHER`, selective `is export(:tag)`. `ValueList` stops being a `std::vector`: one-pass relocating growth and a per-capacity free list under the argument list — 2–29% fewer instructions than the shipped v3.24.0 on 20 of 26 kernels, and the 1–2% it still loses on `asg`/`loopsum` bisected to one `thread_local` store per statement, named and not fixed. Roast **199,980 / 219,403 (91.1%)**, **651 / 1,464 files** (band 651/651/650, union diff empty in the regressed direction, four gained); regression suite **637**; battery 50/59; documentation examples **953**; ecosystem 746 carried forward, no sweep this cycle. The perf gate: seven call-shaped kernels 9–18% faster, none over tolerance; the baseline re-record **refused twice on run spread and was not forced** — the one checklist item left open. |
| 2026-09-08 | **v3.26.0** | **the Grand Review, and what the gates found after it** — three review phases (all of `src/`, then every user-facing document, then the Internals book) over 118 commits; the source phase alone moved Roast 649 → 660 across 21 gated batches, the docs phase closed 204 findings and spun off three engine bugs rather than documenting them as limitations, and the book phase found `docs/book` and `docs/internals` were frequently the SAME document (eight internals pages are now stubs pointing at the chapters that absorbed them). The release's own lesson is about the instruments: gate 1 passed on the file LIST and was wrong to, and the per-file **denominator** join found four faults behind it — `qw:v[…]` had stopped allomorphing (#69's correct fix dropped the one EXPLICIT spelling along with the implicit ones; `allomorphic.t` 87 → **108** of 119, better than the 98 it had before the review, the adverb having masked work already landed), and six IO path methods answered for invocants that are not paths, so `Any.basename` was `"/"` and `Any.contents` LISTED THE CWD — invisible until #62 made an unlistable path an honest `X::IO::Dir` and four Roast files that had been walking past a hole died mid-run. Gate 6 went 50 → 47 on three dists that all broke in the 55 commits BEFORE the review: Test::Output (`UNIT::{…}:exists` never reached the pseudo-package adverb branch) and File::Temp (`IO::Handle.path` returned the stored Str, not an IO::Path — two copies of the arm, the earlier shadowing the later) are fixed; **Cro::HTTP is not**, and ships at 49/59: its four DATA-frame rows end at an `await` on the flow-control promise that never resolves when the serializer's supply was set up off the main thread, and six attempts to reduce it to a synthetic case all pass. Roast **200,220 / 219,429 (91.2%)**, **661 / 1,464 files** (four runs 661/661/662/662, union gains 13 and loses one timeout artifact, `S17-scheduler/basic.t`, which passes 34/34 in 0.02 s alone); regression suite 637 → **798**; documentation examples **956**; operator divergences 23 → **21**; ecosystem 824 carried forward. The perf gate went red and the reading did not support it: a binary built BEFORE the review fails it too, on a different kernel, and head to head the two agree within ±3.3% — **the baseline is still v3.24.0's, un-re-recorded for a third release**, because this box no longer reaches the state its own baselines were taken in. |
| 2026-09-11 | **v3.27.0** | **Windows becomes a platform, and the code other people wrote** — 74 commits, thirteen of them Windows, which until now was a build target rather than a platform anyone could use: `$*DISTRO.is-win` was a hard-coded False on EVERY host (so `tools/install.raku`, `t/run.raku`, `perf-guard` and `Gate.rakumod` all believed they were on POSIX), and the blind FFI path — the one Windows always takes, shipping no libffi — held eight integer arguments where `CreateWindowExW` takes twelve, and passed every value as a 32-bit `long` under LLP64, so a real `HMODULE` came back as `-2003238912`. Sixteen arguments and machine words now, plus an installer (one-liner and Inno wizard, both CI-gated). The rest came from running other people's code: `rakupp install App::Rak` reaches Needle::Compile (16 of 18 dists, `rak` runs), Red's `model` declarator works via `EXPORTHOW::DECLARE`, `BUILDALL` walks the MRO (#72 fez) without giving composed roles an ancestor's turn (#75 Sparrow6), and every `X::` exception finally has the 383 ancestry relations Rakudo declares — `.^mro` used to answer `Any, Mu` on all of them, so `when X::TypeCheck` never fired. **This release ships two regressions, both found by the gates.** `OO::Monitors` no longer excludes — four threads give 3,931 instead of 4,000, deterministically, bisected to `7f09744`, the same commit that added the declarator support Red needed. And the call path is ~10% slower than v3.26.0 (`fib` +10.1%, `subcall` +5.5%, `strpass` +5.9%, against `objnew` **−36.1%**) — which **gate 3 could not see**, because against its four-release-stale v3.24.0 baseline `fib` reads −1.6%; the cause is unlocated, layout ruled out by measurement. The baseline refused re-recording a third release running, naming a different kernel each of three attempts. Roast **200,432 / 219,558 (91.3%)**, **669 / 1,464 files** (four runs 669/669/668/670, union 663 → 670, gaining nine and losing two files that had been scored full passes while dying early); regression suite 798 → **855**; documentation examples **955**; operator divergences **21**; battery 49 → **48 / 59**; ecosystem 824 carried forward. |
| 2026-09-12 | **v3.28.0** | **RakuAST, a formatter, and Raku in your own language** — 33 commits. **RakuAST is in, end to end**: the 489-class hierarchy plus `.AST`, `.DEPARSE`, `.EVAL`, `visit-children` and `.rakudoc`, built as a **view on demand** over rakupp's own parse rather than as a second front end — which is the whole design, and why the ordinary path pays nothing for it (worst +2.7%, mean **−1.3%** over sixteen kernels against v3.27.0 built from source and interleaved). Measured against Rakudo's own trees over the raku-corpus programs both engines can tree, the view carries **72.9%** of the nodes; `rakupp --rakuast` prints ours and `tools/rakuast-oracle-dump.raku` prints Rakudo's, so the comparison is a `diff`. Four things measurement said NOT to build are recorded in the plan, including `@*LINEAGE` (the walker's, not the engine's) and `.parent` (public API upstream, answers `Nil` for every tree a program can obtain). On the same keyword table, **`use L10N::XX;` writes a whole program in one of eleven languages** — German, Japanese, Afrikaans — which upstream is a slang and here is a rewrite of the token stream between the Lexer and the Parser, because an L10N slang renames keywords rather than changing the grammar. Rakudo 2026.08 needs `RAKUDO_RAKUAST=1` for those modules; this engine needs no flag. **`rakupp --fmt`** formats source in the house style: whitespace only, R1-R6, with three gates on every run (it parses, it is the same program, formatting it again changes nothing) — the wide sweep put it at 1 gate refusal in 757 repo files, 0 in 954 raku-corpus programs and 10 in 723 installed ecosystem modules, all one recorded family. **This release ships the same two regressions v3.27.0 named, both unmoved and both re-measured**: `OO::Monitors` still gives 3/5 where Rakudo gives 5/5 (#78), and the call path is still v3.27.0's (`fib` −0.4%, `subcall` −0.7%, `strpass` −1.4% head to head, so #79 is neither fixed nor worse). The perf baseline refused re-recording for a **fourth** release — `--check` came back INCONCLUSIVE on *both* binaries, worst spread 24.2% on HEAD and 28.2% on v3.27.0 against a gate that fires at 5%, red on different kernels each, which is the ambient signature and not code. Roast **200,504 / 219,555 (91.3%)**, **670 / 1,464 files** (three runs 670/670/669, union **identical** to v3.27.0's — 0 regressed, 0 gained — plus a per-file denominator join against v3.27.0 built from the tag, because the release kept no `roast.txt`: nothing worse in all three runs); `rakupp upgrade` fixed: it `use`d the ecosystem `Digest::SHA256::Native` where the engine answers `Digest::Native` itself, so it died before doing anything on any box without that distribution — CI red on all three platforms since the installer batch, invisible here because this machine has it. Roast regression suite 855 → **868**; documentation examples 955 → **957**; operator divergences **21**, unchanged; battery **48 / 59**, the same four DIFFs as recorded; ecosystem 824 carried forward. |
| 2026-08-29 | **v3.23.0** | **the re-baseline** — the third and last consolidation release. Scoped as "re-measure every figure through the instruments v3.22.0 fixed"; it began by reviewing those instruments and found **twenty-five more defects** ([TOOLS-3.23.md](../dev/findings/TOOLS-3.23.md)), three of which would have corrupted this release's own numbers. **Nothing recorded what was being measured**: the gates are a PATH lookup and three rakupp binaries answer to that name here (v3.22.0, v1.0.0, v0.5.1) — measured on one Roast file, v1.0.0 reports 0/2 where v3.22.0 reports 1/2 — and the Roast checkout was unversioned although gate 1 is a diff against the last release's list. Both are now in the run's banner and a `.meta` sidecar, behind a new **gate 0**. **The documented build command produced a translated binary**, because `/usr/local/bin/cmake` is x86_64 and CMake infers the target from its own process — one fact upstream of three gate defects; `CMakeLists.txt` now asks the kernel. **The irreversible half of the perf gate had no guard**: `--check` refuses a noisy machine three ways, `--record` refused nothing, and the test that proved it recorded from a `-modified` build. `tools/lib/Gate.rakumod` replaces six copies of the binary-picking code in four forms. One engine bug: a `:g` match did not carry its subject, so `.orig`/`.prematch` answered the matched text and `.from` counted **bytes** — right by accident on ASCII. And one claim corrected: v3.22.0's "every gate detects a planted defect" is not true, because **conformance has no red path at all** — which is the check producing README's documentation-example figure. Roast **198,939 / 218,773 (90.9%)**, **643 / 1,464 files** (band 640/643/643, union diff empty both ways); regression suite **580**; battery 50/59; documentation examples 950. The perf baseline was re-recorded and **absorbs a second consecutive release of unexplained drift** — `best` keeps every earlier figure, so `vs best` still reads `rats +39.3%`. NOT done: the 2,524-dist ecosystem sweep (stopped at 55; the 637 figure is carried forward, not re-measured), and the source review is still three files of eighty-three. |
...
| 2026-09-17 | **v4.0.0** | **Raku that travels** — the major this version number was reserved for on 2026-08-08, a month before the code: `VERSIONS.md` named it and set three pillars, and this is the tag that collects them. **Modules that travel** — `rakupp install` resolves the fez index, verifies checksums, runs each distribution's own suite and writes the shared CURI store, so getting a module no longer needs Rakudo and zef; and a compiled binary carries its modules with a *guarantee*, every mode reporting what it embedded and what it could not and why, with `--standalone` turning any skip into a build refusal. `%?RESOURCES` was the last piece and landed in this cycle, and the measurement was worse than the plan's description of the gap: it was not merely unembedded but **empty in every compiled binary**, so `%?RESOURCES<x>` answered `Any` and the program died on a message that never mentions resources — with the distribution still on disk, so it was never only a travel problem. The plan said to serve the payloads from memory; that was wrong, because `is native(%?RESOURCES<libraries/x>)` hands its value to `dlopen`, so a dist materialises its resources into one temp directory on first use. **rakupp as a library** — one C API with lifecycle, eval, output capture and rooted handles, and bindings over it for C++, Go, JavaScript, Python, Rust and Wolfram. **Grammars as a service** — the grammar stays a `.raku` file and each host class is a generator over that text, never a parallel path, which is what keeps the API from owing a maintenance debt that grows with Raku itself. Beyond the pillars, 103 commits, and an honest denominator for the ecosystem: every distribution rakupp does not pass, re-run **under Rakudo on this machine through our own harness**, puts the ceiling at **1,791 of 2,529** and shows **738 that cannot pass here under any engine** — so 1,006 is 40% of the catalogue and **56% of what is reachable**. **The release's own gates found five engine faults and fixed them**: an infinity comparing equal to its opposite (two zero-denominator Rats cross-multiply to 0 against 0, so every ordering between them answered Same); a `::T` type capture that named a type and constrained nothing; a primed routine answering arity 0 while its own `.signature` answered 1; a type object's own `Bool` ignored in every boolean context (`so C` False where `.Bool` was True); and a subclass of `Parameter`/`Attribute` never receiving its own declared attributes. Two of the five came from the **denominator join**, which is the only check that sees a file losing assertions without leaving the pass list — its other three entries were run down rather than recorded: one is the machine (a `todo` shield keyed on macOS release *names*, reproduced by a v3.25.0 engine on the new OS), one is punned parameterized roles and is written up rather than patched, and one **was passing for the wrong reason** — `whenever $socket` used to run its body once with the socket as topic instead of tapping it, so making that correct removed an accidental pass and exposed a real pre-existing gap beneath. **The perf baseline is current again after four releases**: it was recorded at v3.24.0 and gated v3.25.0–v3.28.0 against a machine four releases in the past, and this release found the bill — `multimeth` is ~17% slower than v3.28.0 on this same box while the gate reported it as a 12.7% *improvement*, because four releases of real gains elsewhere had moved the comparison out from under it. Bisected to the commit that made multi narrowness per-parameter rather than a sum (a correctness fix: a summed score let `(Str:D, Str:D)` outrank `("not", Any:D)` and steal every call), then partly recovered — 22–26% down to 17.2% — by hoisting a per-candidate allocation out of the dispatch loop and caching a hashed type lookup on the Param. The rest is the price of correct narrowness and is recorded as such. **Shipped with one serious defect named rather than hidden**: a data race in `evalCall`'s name lookup segfaults the process when one sub is called from several promise threads at once, ~44% of runs on a nine-line repro, present in v3.26.0–v3.28.0 and not in v3.25.0. Roast **676 / 1,464 (46.2%)** and **200,843 / 219,610 (91.5%)** — four runs identical, on a harness repaired mid-cycle that had been inheriting an unclosed stdin pipe and counting files that finish in 0.1 s as timeouts; union diff against v3.28.0 **0 regressed, 6 gained**, every gain verified as the engine rather than the instrument. Local suite 868 → **1,020**; battery 48 → **50 / 59** with 0 DIFF, `OO::Monitors` (#78) and `AttrX::Mooish` both DIFF → PASS; documentation examples **957** and operator divergences **21**, both unchanged; ecosystem 989 → **1,006** on a board with 296 dists re-measured, 21 gained and 4 lost, none of the four an engine regression. |
| 2026-09-17 | **v4.0.1** | **`::T:U` constrains, it does not capture** — a fix-only tag hours after the major. A smiley decides what a `::` parameter is: `::Foo $x` is a type **capture** and binds `Foo` to the argument's type for the body, while `::Foo:U $x` is an ordinary **constraint** and must leave an outer `Foo` visible. v4.0.0 read both as captures — it had just fixed the bare capture, one of that release's five faults, and applied the new reading one form too wide — so a signature that merely constrained a parameter silently rebound that type's name for the whole routine. YAMLish is the case that found it: `load-yaml(… ::Grammar:U :$schema)` goes on to call `Grammar.parse`, meaning the module's own grammar and the `method parse` override that attaches its actions; captured, the call reached `$schema`'s type and the plain built-in parse instead, so the match still succeeded, no action method ever ran, and `.ast` came back as the document's own source text — every `load-yaml` dying on `No such method 'concretize' for invocant of type 'Str'`, a message naming neither the signature nor the grammar. Caught by raku.online's `sites/spec/verify.sh` on a documentation example that passed at v3.25.0, and confirmed against the **released** v4.0.0 artifact rather than a local build. The shape appears in 329 battery files and in 43 of those the captured name is also used in the body; only YAMLish is a measured failure, the rest are candidates. Roast **unchanged** — 676 / 1,464 and the same file list byte for byte. One divergence is deliberately left standing: rakupp enforces the smiley, refusing `f(42)` for `sub f(::Foo:U $x)` where Rakudo binds it — v3.25.0 refused the same cases, so it predates the type-capture work and the regression test records it rather than asserting it. Also `tools/run-roast.raku` gains `-j=N`, which sets the CPU budget and sizes the worker pool to match, defaulting to every core rather than one fewer. |
