...
| **Startup** | ~2 ms cold |
| **Correctness target** | the Roast suite — ~90% of all individual tests pass; ~42% of files fully pass |
...
- **All declared tests: ~90%** (196,381 / ~217,101) — the headline per-test figure.
  ...
  files can't hide.
- **Files fully passing: ~43%** (622 / 1,462) — the stricter bar; a file counts
  only if *every* assertion in it passes.
- **Tests that ran: ~98%** (196,381 / 200,501) — of just the assertions files
