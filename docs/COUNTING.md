...
> **Per-test: ~87% of all declared tests pass. Coverage: ~36% of files fully pass.**

Quote both, per-test first. The ~87% is the primary correctness number (the fair
per-test bar); the ~36% is the stricter all-or-nothing file bar.
...
|---|---|---|---|
| 1 | **Files fully passing** | 528 / 1,462 (**~36%**) | a file counts only if *every* planned assertion passes (or it legitimately `plan skip-all`s) |
| 2 | Assertions of **tests that ran** | 186,778 / 193,235 (~97%) | numerator ÷ assertions the files actually emitted |
| 3 | Assertions of **tests planned** | 186,778 / 206,946 (~90%) | ÷ the plan `N` of every file that emitted a plan (so tests lost to a mid-file abort count against us) |
| 4 | Assertions of **all declared tests** | 186,778 / 214,528 (**~87%**) | ÷ every test any file declares — including files that abort before emitting TAP, whose `plan N` is read from source |

**Measure 1 (files, ~36%)** and **measure 4 (all declared tests, ~87%)** are the
...
source and counts all N as failing. That is why 4's denominator (214,528) is ~7.6k larger
than 3's (206,946) — those ~7,600 tests live in 152 no-TAP files (parse errors
...
denominator.** Our current run recovers **214,528** declared tests. (This number
...
So our same 186,778 passes read two ways:

- **~87%** against *our* denominator (186,778 / 214,528) — *"of the tests we can
  account for, how many pass."* This is what a single harness run can measure,
  and it is the number we quote.
- Essentially the **same ~87%** against the suite's *full* declared total —
...
headline **~87% is no longer materially flattered** by uncountable files —
...
```
Files fully passing:  528 / 1462  (36.1%)
Assertions passed:    186778 / 193235  (96.7%)  of tests that ran
Assertions passed:    186778 / 206946  (90.3%)  of tests planned by files that emitted a plan
Assertions passed:    186778 / 214528  (87.1%)  of ALL declared tests (+7582 from 152 no-TAP files read from source; 5 more have no static plan)
```
