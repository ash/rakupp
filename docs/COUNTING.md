...
|---|---|---|---|
| 1 | **Files fully passing** | 630 / 1,462 (**~43%**) | a file counts only if *every* planned assertion passes (or it legitimately `plan skip-all`s) |
| 2 | Assertions of **tests that ran** | 196,568 / 200,815 (~98%) | numerator ÷ assertions the files actually emitted |
| 3 | Assertions of **tests planned** | 196,568 / 214,129 (~92%) | ÷ the plan `N` of every file that emitted a plan (so tests lost to a mid-file abort count against us) |
| 4 | Assertions of **all declared tests** | 196,568 / 217,060 (**~90%**) | ÷ every test any file declares — including files that abort before emitting TAP, whose `plan N` is read from source |
...
So our same 196,568 passes read two ways:

- **~90%** against *our* denominator (196,568 / 217,060) — *"of the tests we can
