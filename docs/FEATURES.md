...
Roast standing: measured per individual test, **~89% of all declared tests pass** (193,500 / ~215,896, counting tests in files that abort before running); on the stricter file bar, **575 / 1,462 fully pass (~39%)** (671 partial, 212 no-TAP, 11 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
