...
Roast standing: measured per individual test, **~87% of all declared tests pass** (186,778 / ~214,528, counting tests in files that abort before running); on the stricter file bar, **528 / 1,462 fully pass (~36%)** (685 partial, 238 no-TAP, 11 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
