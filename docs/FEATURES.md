...
Roast standing: measured per individual test, **~90% of all declared tests pass** (196,381 / ~217,101, counting tests in files that abort before running); on the stricter file bar, **622 / 1,462 fully pass (~43%)** (687 partial, 144 no-TAP, 9 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
