...
Roast standing: measured per individual test, **~90% of all declared tests pass** (193,883 / ~216,066, counting tests in files that abort before running); on the stricter file bar, **578 / 1,462 fully pass (~39%)** (671 partial, 212 no-TAP, 11 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
