...
Roast standing: measured per individual test, **~90% of all declared tests pass** (196,590 / ~217,055, counting tests in files that abort before running); on the stricter file bar, **631 / 1,462 fully pass (~43%)** (679 partial, 143 no-TAP, 10 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
...
- **NativeCall**: `sub … is native {*}` / `is native('lib')` / `is native(&lib-sub)` / `is symbol('n')` calls a C function via `dlsym`, through a fixed 8-integer/8-float prototype that lets the platform ABI place the arguments (the resolved symbol is cached per sub — the `dlopen` candidate scan is the expensive part)
  ...
  - Callbacks: a Raku `Callable` passed to C becomes a function pointer via a pool of 64 trampolines — `qsort`/`bsearch` work. Up to 6 integer/pointer/`num` arguments, integer return, and the callback must fire *during* the native call (the GIL is held)
  - **Gaps** (no `libffi`): **variadic C functions are called incorrectly, without an error** — their arguments are passed in registers rather than where the ABI wants them, so `snprintf($buf, 16, "%d", 42)` yields `"0"`, and `curl_easy_setopt` and friends cannot work; by-value struct *arguments* and struct returns wider than a register pair are unsupported (an 8-byte return can be taken as `int64` and unpacked by hand); more than 8 integer or 8 float register arguments is a clean error; callbacks that C stores and fires later from its own thread are not supported
- `$*CWD $*EXECUTABLE $*ARGS $*RAKU/$*PERL` (`.compiler.name` = "Raku++", backend "cpp"; `.compiler.version` deliberately reports the **Rakudo era tracked**, `v2026.07`, since modules gate features on it — the Raku++ release is `.release`/`.id`), `$*DISTRO $*KERNEL $*VM $*THREAD $*SCHEDULER`
