...
Roast standing: measured per individual test, **~80% of all declared tests pass** (168,848 / ~211,845, counting tests in files that abort before running); on the stricter file bar, **448 / 1,462 fully pass (~31%)** (692 partial, 316 no-TAP, 6 timeout). See [COUNTING.md](COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
