...
**Headline: ~80% of all declared Roast tests pass** (169,885 / 211,886); on the
stricter file bar, ~32% of files fully pass (467 / 1,462). The per-file breakdown
...
**169,885 of ~211,886 declared tests — ~80%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 169,885 / 176,193 (~96%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 169,885 / 202,435 (~84%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 169,885 / 211,886 (~80%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
_Snapshot: 467 / 1,462 files fully passing (~32% coverage); 675 partial,
315 no-TAP, 5 timeout. Reached-assertion pass rate 169,885 / 176,193 (see
