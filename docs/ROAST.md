...
**Headline: ~90% of all declared Roast tests pass** (194,264 / 216,066); on the
stricter file bar, ~38% of files fully pass (582 / 1,462). The per-file breakdown
...
**194,264 of ~216,066 declared tests — ~89%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 194,264 / 198,906 (~97%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 194,264 / 209,990 (~92%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 194,264 / 216,066 (~89%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
212 no-TAP, 11 timeout (the scheduler/io timing files flap between pass and timeout under runner load). Reached-assertion pass rate 194,264 / 198,906 (see
