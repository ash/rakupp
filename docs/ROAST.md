...
**Headline: ~80% of all declared Roast tests pass** (170,024 / 211,886); on the
stricter file bar, ~33% of files fully pass (479 / 1,462). The per-file breakdown
...
**170,024 of ~211,886 declared tests — ~80%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 170,024 / 176,236 (~96%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 170,024 / 202,394 (~84%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 170,024 / 211,886 (~80%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
_Snapshot: 479 / 1,462 files fully passing (~33% coverage); 662 partial,
315 no-TAP, 6 timeout. Reached-assertion pass rate 170,024 / 176,236 (see
