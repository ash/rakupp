...
**Headline: ~90% of all declared Roast tests pass** (194,901 / 216,222); on the
...
**194,901 of ~216,222 declared tests — 90%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 194,901 / 199,872 (~98%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 194,901 / 212,964 (~92%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 194,901 / 216,222 (90%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
158 no-TAP, 11 timeout (the scheduler/io timing files flap between pass and timeout under runner load). Reached-assertion pass rate 194,901 / 199,872 (see
