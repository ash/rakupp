...
**Headline: ~80% of all declared Roast tests pass** (168,848 / 211,845); on the
stricter file bar, ~31% of files fully pass (448 / 1,462). The per-file breakdown
...
**168,848 of ~211,845 declared tests — ~80%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 168,848 / 174,885 (~97%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 168,848 / 202,384 (~83%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 168,848 / 211,845 (~80%) | + tests in parse-error files, recovered from source. This denominator grows as parse fixes land — files that died before announcing a plan now declare their real (often larger, dynamic) plans, so the percentage can dip while absolute passes rise |
...
_Snapshot: 448 / 1,462 files fully passing (~31% coverage); 692 partial,
316 no-TAP, 6 timeout. Reached-assertion pass rate 168,848 / 174,885 (see
