...
- **A role's required-method stub is not enforced at composition.** `role Shape { method area { ... } }` then `class Triangle does Shape { }` (no `area`) — Rakudo refuses to compile: `Method 'area' must be implemented by Triangle because it is required by roles: Shape`; rakupp accepts the class silently (exit 0, no diagnostic). The whole contract value of a role stub is lost.
...
- **A read-only accessor / private attribute is writable from outside.** `class Account { has $!balance = 0; method balance { $!balance } }; my $a = Account.new; $a.balance = 1000; say $a.balance;` — Rakudo dies with `Cannot modify an immutable Int`; rakupp lets the assignment through and rewrites the private `$!balance` (prints `1000`). `$obj.method = v` treats the method's *result* as an lvalue, and because a private `$!x` is stored under its bare name, the write lands straight in the private slot — so the encapsulation / read-only boundary is not enforced. Same for a public read-only accessor (`has $.x; $o.x = 5`). *Not yet fixed:* the naive guard (allow only public `is rw` accessors) regresses genuine `is rw` / `return-rw` lvalue methods (`method l1 is rw { return-rw $!a }`) and mutation-through-accessor (`$o.mda[i;j] = v`), so a correct fix must distinguish those — more than a one-line change.
