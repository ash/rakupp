...
### 3. Benchmarks — rakupp vs Rakudo, continuously
...
- **Trend charts** — PWC match % of counted, eco parse-clean/suite-green
  counts, benchmark ratios per kernel — as static SVG generated by the report
...
Rakudo → fetch the two corpus deltas → run the three measurement legs →
...
3. **The driver** — `tools/eye-run.raku`: orchestrates the four legs, owns
...
- **`raku-corpus` as a third correctness leg** — the published real-world
  corpus is small and stable; a monthly re-run would catch regressions the
  Roast gate misses. Cheap to add once the driver exists; not in week zero.
