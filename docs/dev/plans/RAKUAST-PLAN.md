...
**The published number: 72.9% — 17,854 of Rakudo's 24,499 nodes, over the 39 of
59 corpus programs both engines can tree** (8 Rakudo itself refuses, 12 the view
refuses by name). The view also builds 5,132 nodes Rakudo's tree has not; that
...
## P4 — traversal, and what the acceptance corpus actually said (landed 2026-09-12)

`visit-children` and the vocabulary growth are in, gated cross-engine. The
acceptance corpus is the part that did not go as the plan expected, and that is
the more useful half of this entry.

**Files**: `visit-children` in RakuAstClasses.cpp, the registry-as-parent arm in
Interpreter.cpp, `nqp::can`, `Parser::typeWordAsTerm`, the generator's full-set
walk, `tools/rakuast-visit-spec.raku` +
`docs/dev/findings/rakuast/visit-2026.08.txt` + `t/regression/rakuast-visit.raku`.

**Tree oracle: 70.1% → 72.9%** (17,854 of Rakudo's 24,499 nodes over the same 39
programs), extra nodes 5,565 → 5,132.

### `visit-children`, and the `@*LINEAGE` that is not ours to provide

One walk over the node's own attribute map, which is insertion-ordered — so the
children come out in the order the view set them, which is source order. The
children are collected BEFORE any is visited: the callback is user code, and
mutating the map while iterating it is the rehash-under-reader crash.

`@*LINEAGE` needed nothing at all, and finding that out was the point of
measuring rather than implementing. It is an ordinary dynamic that the WALKER
declares as it descends — Rakudo does not populate it inside `visit-children`
either. Nor is `.parent` the thing to build: it is public API upstream
(`parent(int $generation?)`) and answers `Nil` for every tree a program can get
hold of, on a plain `.AST`, on `:compunit`, and after the tree has been compiled
and run. Our nodes are refcounted, so a stored parent link would be a cycle and
a leak; the API the ecosystem actually uses costs us nothing.

The gate is the same shape as deparse's and eval's: one engine-neutral spec file,
run under Rakudo into a committed file, run here, and the diff must be empty.
Its programs are chosen to isolate the traversal from the vocabulary — the two
recorded shape differences (a declaration's `desigilname`, a parameter's implied
declaration) stay out of them deliberately, and the `BEGIN` block has an empty
body because Rakudo RUNS it during `.AST` and we do not.

### The class table is now every class the namespace has

199 → **489**, generated by walking Rakudo's own stash rather than growing a
demand list one `Undeclared name` at a time. ASTQuery alone references most of
the namespace when it compiles its matcher tables, and each miss surfaced at the
user's end rather than as a gap in a walk. Cost, measured: **+16 KB** of binary
and no change to startup — the registry is still built once, published once, and
consulted only after `classes_` has missed.

**The generator was hiding a real bug.** A name it could not look up got a row
with an EMPTY ancestry instead of a complaint — which is how `Statement::While`
sat in the table as a class rakupp had INVENTED (a plain `while` is
`Statement::Loop::While` upstream), with no path to `RakuAST::Node` and therefore
none of its methods; `visit-children` was simply missing on every `while`
statement. It now prints what it could not resolve, both to stderr and into the
generated file, because two different things land there and only one is a bug:
`Statement::Elsif` and `Statement::Orwith` are real nodes in real trees that
Rakudo does not expose as symbols, and those belong in the table.

Two traps in writing that check, both of which marked all 491 names as missing:
a type object is not `.defined`, and `Nil` has a `.^name`. The test is a Failure
out of `::()` or a Nil out of the stash walk, and nothing else.

### Three more the traversal gate found

1. **A `SemiList`'s elements are STATEMENTS.** Every composer — `[ ]`, `( )`, a
   subscript, a contextualizer — put the bare expression in, dropping a
   `Statement::Expression` per composer.
2. **`{…}` in a string is a BLOCK.** An interpolated segment is a whole
   `Block → Blockoid → StatementList → Statement::Expression` upstream; ours was
   the bare expression. The renderer unwraps it again — the braces are the
   interpolation's own.
3. **`class`/`role`/`grammar` could not be used as TERMS.** `nqp::eqaddr(type,
   role)` stopped an argument list dead ("expected ) (got 'role'"), and the
   quieter half is that `say role` had been parsing as an anonymous role
   declaration and printing a blank line. A declarator must be followed by a
   name, a `{` or a `::`; anything else means the word is being used. This is a
   parser fix, not a RakuAST one — RakuAST::Utils is just where it showed up.

### The acceptance corpus, run rather than grepped

The plan named three modules off a source grep. Running them says something
different, and each is now blocked for a precisely different reason — none of
them traversal:
...
* **FINALIZER needs `$*R.find-attach-target`** — Rakudo's compile-time Resolver,
  i.e. being a RakuAST frontend, which Part I rules out. What P4 did fix is the
  half that was ours: `my class LeavePhaser is
  RakuAST::StatementPrefix::Phaser::Leave` now resolves, because the registry
  lives outside `classes_` and was reachable from every name position except
  the inheritance one.
* **RakuAST::Utils** is blocked on `.^roles` over a BUILT-IN type — `Int`,
  `Array`, `Array[Int]` all throw here and all answer upstream. A metamodel gap
  with nothing RakuAST about it. Past it are only the ordinary nqp ops; `nqp::can`
  was the last one missing and is in.

So P4's vocabulary growth landed and its acceptance modules did not, for reasons
that are worth more than a green tick would have been: one module is broken
upstream, one wants the design Part I rejected, and one wants a feature in
another subsystem entirely.

### Gates

`t/run.raku` 864/864, `t/slim/run.raku`, the six RakuAST regression cases
