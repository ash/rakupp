...
> `rakupp install` (a command-line rewrite in main.cpp; the script shipped in
> the install layout's `libexec/rakupp/`, found beside the binary, and a
> checkout used `tools/` — superseded 2026-09-09, the script is now baked into
> the CLI and nothing is looked up: see the second revisited note below).
> What deviates from the sketch below, and why:
>
...
>
>   *(This half was NOT taken — see the note below.)*
...
> happened. `tools/gen-embedded-tools.raku` writes `src/EmbeddedTools.cpp` —
> `tools/install.raku` as `installerSource()`, `tools/doc.raku` as
> `docToolSource()` with `docs/guide/{REFERENCE,FEATURES}.md` spliced into its
> `%DOCS` declaration, in 15 KB raw-string chunks because MSVC caps a literal
> at 16 KB. The file is checked in, not generated by CMake: generating it needs
> a working rakupp, which is the thing being built. It joins `Js.cpp` and
> `JsRuntimeSrc.cpp` in the CLI-only group, so `librakupp_rt.a` carries none of
> it and no `--exe` binary does either; `t/slim/run.raku`'s budgets are
> measured on `--exe` output and are untouched.
>
...
> blob was that a checkout could edit the script without a rebuild — but the
> two ship in one release every time, so the only thing the precedence rule
...
> and editing `tools/install.raku` means regenerating. `t/install/run.raku`
> runs `--check` and fails on a stale blob, then copies the binary into an
> empty directory and runs `install` and `doc` there — the case that used to
> exit 4. (`RAKUPP_DOCS=DIR` still overrides the baked guides for `rakupp doc`;
> it is an explicit act, not a file lying around.)
