...
| B7 | UUID / ULID | `Data-UUID` CPAN, `uuid` stdlib/npm | `UUID::V4` (fez) only; `UUID`, `LibUUID`, `ULID` REA-only | add v1/v5/**v7** + ULID; trivial, pure Raku — planned in [UUID-PLAN.md](../plans/UUID-PLAN.md) |
