...
Homebrew Rakudo v2026.08. Compared against Raku++ 4.0.1-23-g80b16d18
(build-arm64, 2026-09-18). Format and legend: [README.md](README.md).

Where this sits against the declared spec: Rakudo passes 20 of 20
`S32-array` files and 50 of 50 `S32-list` files here; Raku++ passes 6 and
23. The rules below are the ones those files, and real programs, keep
tripping over: the single-argument rule at every entry point, what a hole
is, which errors are Failures and which are thrown, what a Seq allows after
it has been iterated, and where `flat` stops. 4 of the 36 items
are neither fully documented nor fully asserted by Roast.
...
# rakudo 2026.08: (...) (...) [...] "..."
```
rakupp 4.0.1: differs — the lazy raku ends `...).lazy.Seq` (404 chars).
...
```
rakupp 4.0.1: differs — `Empty === Empty` and `slip() === Empty` are False; `Empty.map` is `().Seq`.
...
```
rakupp 4.0.1: differs — the first assignment throws `X::Assignment::RO` past the `try`, so the line died; the rest is unmeasured.
...
```
rakupp 4.0.1: differs — `for (1, 2) { $_++ }` is accepted; `.values` yields copies (`[1, 2]`).
...
```
rakupp 4.0.1: differs — `@a[0] := @a[1]` does not alias (2); the list binding is writable; a Seq and `42` bind.
...
```
rakupp 4.0.1: differs — `$l.Array[0] = 9` dies "Target is not assignable" (`.Array` of a List yields no containers).
...
```
rakupp 4.0.1: differs — a lazy list gives `\()`.
...
```
rakupp 4.0.1: differs — the nested case gives `"<1>,<2 3>"`; two directives give `"1 and 0 2 and 0"` silently.
...
```
rakupp 4.0.1: differs — a lazy sum throws instead of failing.
...
```
rakupp 4.0.1: differs — joining a lazy list dies ("Cannot join a lazy list onto an Array").
...
```
rakupp 4.0.1: differs — a delete on a lazy array shifts the rest (`(1, 3, 4, 5)`, `:exists` True); a typed hole prints as Any.
...
```
rakupp 4.0.1: differs only in the unknown-adverb type (`X::Adverb`).
...
```
rakupp 4.0.1: differs — `@a[*] = 0` gives `[0, 2, 3]`, `@a[*-1, 0] = 0, 9` gives `[9, 2, 3]`, `@a[*] = 7, 8, 9, 10` gives `[(7, 8, 9, 10), 2, 3]`.
...
```
rakupp 4.0.1: differs — `@a[0;0]` on a scalar element gives Any.
...
```
rakupp 4.0.1: differs — `@a[1][1]` gives Nil, an out-of-shape write is accepted (`no`) or `X::OutOfRange`, a one-dimensional shaped array prints as a plain `[1, Any]`, `Array.new(:shape(2))` prints as `[Any, Any]`, a Range shape is accepted, no warning for a type shape.
...
```
rakupp 4.0.1: differs — `.raku` never shows the type, `my Int @a; @a.default` is Any, `.name` is `element`, a typed hole prints as Any, `Array[Int].new("a")` is accepted, `my @a is default(7)` prints holes as Any.
...
```
rakupp 4.0.1: differs — the lazy cases are accepted; `append({a => 1})` keeps the Hash as one element; `append(%h)` adds one element.
...
```
rakupp 4.0.1: differs — `pop` on a lazy array throws ("Cannot pop a lazy list onto an Array") and killed the line; the rest is unmeasured.
...
```
rakupp 4.0.1: differs — no error for an offset past the end (`[]`), a negative size (`[]`), or a negative offset (`[3]`, counted from the end); a lazy replacement is accepted (`[1]`); `[].splice(1)` is `[]`; the removed array's `.of` is Mu; splice on a lazy array is accepted.
...
```
rakupp 4.0.1: differs — `X::Method::NotFound`.
...
```
rakupp 4.0.1: differs — writing into a lazy clone loses the reified values before it (`(Any, Any, 0)`).
...
```
rakupp 4.0.1: differs — `elems`, `reverse`, `sort`, `tail`, `join`, `pop` on a lazy list all throw ("Cannot … a lazy list onto an Array") instead of failing, and `pick` gives Nil.
...
```
rakupp 4.0.1: differs — a Seq is never consumed (`$s.sink; $s.elems` is 2) and `Seq.new.iterator` prints garbage (`items`).
...
rakupp 4.0.1: differs — `.rotate` returns a List; `Seq.new.raku` is `()`; the lazy `.elems` throws.
...
rakupp 4.0.1: differs — the nested group destructures (`(1, 2, Any, 3)`), a typed target resets to Any, and `(1, 2), 3` flattens to `(1, 2, 3)`.
...
```
rakupp 4.0.1: differs — `++$n xx 3` dies with `X::Cannot::Map`.
...
```
rakupp 4.0.1: differs — `zip(:with)` returns a List, `infix:<X>(…, :with(…))` puts the named argument into the tuples, `((1..*) X (3, 4)).is-lazy` is False.
...
```
rakupp 4.0.1: matches in shape (the `$` markers are absent in its `.raku`).
...
```
rakupp 4.0.1: differs — `sort(:k)` returns a Seq; the rest matches.
...
```
rakupp 4.0.1: differs — `.WHICH` is a Str.
...
| rakupp 4.0.1 matches | 5 |

Recurring rakupp gaps, for step two: every lazy-list operation that Rakudo
turns into a Failure or `X::Cannot::Lazy` is a plain throw here, with an
"onto an Array" message; `splice` never validates its offset or size;
`push`/`append` accept lazy sources; a Seq is never consumed; `.raku`
never shows an array's type or the `$` itemization marker; `for` over a
List of values is writable; `:=` between elements does not alias.
