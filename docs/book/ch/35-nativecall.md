...
`RAKUPP_FFI=0` forces this path, and the fallback is tested through it: the
regression test `t/regression/nativecall-libffi.raku` re-runs the cases only
libffi can do in a child process with the backend switched off, and checks each
one throws rather than answering. WebAssembly takes the fallback by
construction — there is no shared library to open.
