...
- **Per-test — ~90% of all declared tests pass (196,052 / ~217,016).** This is the
  ...
  (196,052 / 200,501) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 611 / 1,462 files fully pass (~42%).** The stricter all-or-nothing
