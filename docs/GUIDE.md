...
- **Per-test — ~82% of all declared tests pass (157,293 / ~191,546).** This is the
  ...
  (157,293 / 163,185) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~28k tests in aborting files.
- **Coverage — 419 / 1,464 files fully pass (~29%).** The stricter all-or-nothing
