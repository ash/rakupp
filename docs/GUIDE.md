...
- **Per-test — ~90% of all declared tests pass (196,395 / ~217,060).** This is the
  ...
  (196,395 / 200,691) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 625 / 1,462 files fully pass (~43%).** The stricter all-or-nothing
