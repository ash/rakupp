...
- **Per-test — ~90% of all declared tests pass (194,904 / ~216,222).** This is the
  ...
  (194,904 / 199,872) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 598 / 1,462 files fully pass (~41%).** The stricter all-or-nothing
