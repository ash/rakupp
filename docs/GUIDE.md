...
- **Per-test — ~90% of all declared tests pass (196,381 / ~217,101).** This is the
  ...
  (196,381 / 200,501) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 622 / 1,462 files fully pass (~43%).** The stricter all-or-nothing
