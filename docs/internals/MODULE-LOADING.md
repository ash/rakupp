...
see **[CACHING.md](../guide/CACHING.md)**. `use XML` costs 16.0 ms cold and 5.7 ms
...
| 4 | The PARSE is cached ([CACHING.md](../guide/CACHING.md)); declarations and `BEGIN` still run every time. | `.precomp` caches the parse *and* the objects compile-time code produced, so `BEGIN` runs once per *compile*. |
...
| Concern | Location |
|---|---|
| Precompiled-AST cache | `AstSerial.{h,cpp}`; `loadModule` (modules) and `rakuppRun` (mainline) — see [CACHING.md](../guide/CACHING.md) |
...
[MODULE-FINDINGS.md](../dev/ecosystem/MODULE-FINDINGS.md) for the ecosystem-compatibility log.
