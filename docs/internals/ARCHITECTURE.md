...
large-stack thread so deep recursion is safe — [MEMORY.md](../guide/MEMORY.md) has the
...
[OPTIMIZATION.md](OPTIMIZATION.md); [NATIVE.md](../guide/NATIVE.md) compares compiled vs.
...
| `say`, any named builtin | `rtCallB(RT, __bfpN, "say", …)` — a per-name pointer resolved once at startup (see [DISPATCH.md](DISPATCH.md)) |
