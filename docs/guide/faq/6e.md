...
| Not the first statement | `Too late to switch language version. Must be used as the very first statement.` | **divergence** — accepted anywhere in the unit, and the whole unit becomes 6.e, including the lines *above* the pragma |
...
| | |
|---|---|
| 6.d | `Null regex not allowed. Please use .comb if you wanted…` — `//` is an empty regex. **Raku++ under 6.d** neither errors nor answers: `say //42` reads `//` as an empty regex match against `$_`, which yields no values, so it prints an empty line and exits 0. In other term positions (`say (//42).raku`) Raku++ does refuse |
| 6.e | `True False` |
...
Raku++ gates the namespace by exactly that rule, with that message, and its
classes carry Rakudo's own ancestry — `RakuAST::IntLiteral.^mro` is the same
thirteen names, so `~~ RakuAST::Expression` and `.does` answer as they do
there. `.DEPARSE` is a later step of the campaign, so the line above still
reports "no such method" here; what a 6.d program sees is the refusal, which is
the half this table is about.
...
| | |
|---|---|
| 6.d | loads, with `Saw 1 occurrence of deprecated code` (Rakudo). Raku++ has no deprecation machinery and loads silently |
...
| `macro m() { … }` | works under `use experimental :macros` | `Experimental macros are no longer supported in Raku 6.e.` Raku++ has no macros in **any** revision, so neither cell applies: the declaration parses as a call to an undefined routine and the program dies at run time |
| `sub foo;` | `X::UnitScope::Invalid` (only `MAIN` allowed) | `X::UnitScope::MustHaveUnit` — needs `unit sub foo;`. Raku++ raises `X::UnitScope::Invalid` in **both** revisions; it has no `MustHaveUnit` |
...
- **`RakuAST`** is a separate campaign, now under way. The `RakuAST::` classes
  exist and are gated exactly as the table above says — but the matrix probe is
  `RakuAST::IntLiteral.new(42).DEPARSE`, and the renderer is a later step, so
  this is still the one row the matrix scores as not implemented.
