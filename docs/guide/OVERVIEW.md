...
- **Operators & metaprogramming** — all six user-operator categories
  ...
  [METAPROGRAMMING.md](../internals/METAPROGRAMMING.md).)
...
One front end, four back ends (details in [ARCHITECTURE.md](../internals/ARCHITECTURE.md)):
...
[OPTIMIZATION.md](../internals/OPTIMIZATION.md)). Any construct the native compiler can't yet
...
- **All declared tests: ~90%** (197,060 / ~217,110) — the headline per-test figure.
  ...
  files can't hide.
- **Files fully passing: ~43%** (633 / 1,462) — the stricter bar; a file counts
  only if *every* assertion in it passes.
- **Tests that ran: ~98%** (197,060 / 201,240) — of just the assertions files
...
- **[ROAST.md](../status/ROAST.md)** — coverage, per-synopsis stats, and how the numbers are defined.
- **[COUNTING.md](../status/COUNTING.md)** — the authoritative definition of every pass-rate figure and how it's computed.
- **[BENCHMARKS.md](../status/BENCHMARKS.md)** / **[OPTIMIZATION.md](../internals/OPTIMIZATION.md)** — speed vs Rakudo; the `-O` optimizer.
- **[ASYNC.md](ASYNC.md)** / **[METAPROGRAMMING.md](../internals/METAPROGRAMMING.md)** — concurrency; language-mutation coverage.
- **[ROADMAP.md](../status/ROADMAP.md)** — done / next.
- **[dev/JOURNEY.md](../dev/JOURNEY.md)** — a memoir of how this was built.
