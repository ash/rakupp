...
- **Per-test — ~90% of all declared tests pass (198,642 / ~218,561).** This is the
  ...
  (198,642 / 205,084) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 633 / 1,464 files fully pass (~43%).** The stricter all-or-nothing
