...
- **Per-test — ~90% of all declared tests pass (197,111 / ~218,647).** This is the
  ...
  (197,111 / 203,540) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 595 / 1,462 files fully pass (~41%).** The stricter all-or-nothing
