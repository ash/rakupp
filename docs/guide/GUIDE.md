...
- **Per-test — ~91% of all declared tests pass (200,220 / ~219,429).** This is the
  ...
  (200,220 / 206,347) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 661 / 1,464 files fully pass (~45%).** The stricter all-or-nothing
...
[ROAST.md](../status/ROAST.md) for a per-section breakdown, and
[COUNTING.md](../status/COUNTING.md) for exactly how every figure is defined and computed.
...
- **[RECIPES.md](RECIPES.md)** — recipes: short, runnable snippets, each verified against `rakupp`.
...
- **[ROAST.md](../status/ROAST.md)** — Roast suite overview and per-section statistics.
- **[COUNTING.md](../status/COUNTING.md)** — authoritative definition of the pass-rate figures.
- **[BENCHMARKS.md](../status/BENCHMARKS.md)** — a fair speed comparison with Rakudo on the shared subset.
- **[OPTIMIZATION.md](../internals/OPTIMIZATION.md)** — the `--exe -O` optimizer: passes, C++ level forwarding, numbers.
- **[dev/JOURNEY.md](../dev/JOURNEY.md)** — a memoir of how this was built: the Roast / real-project / docs loops, the clean-room stance on Rakudo, and reaching `--exe`.
...
binary that bundles the interpreter. See [ROADMAP.md](../status/ROADMAP.md).
