...
- **Per-test — ~90% of all declared tests pass (198,628 / ~218,626).** This is the
  ...
  (198,628 / 205,087) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 630 / 1,462 files fully pass (~43%).** The stricter all-or-nothing
