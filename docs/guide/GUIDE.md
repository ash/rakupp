...
- **Per-test — ~90% of all declared tests pass (196,590 / ~217,055).** This is the
  ...
  (196,590 / 200,826) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 631 / 1,462 files fully pass (~43%).** The stricter all-or-nothing
