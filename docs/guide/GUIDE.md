...
- **Per-test — ~90% of all declared tests pass (198,791 / ~218,608).** This is the
  ...
  (198,791 / 205,091) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 638 / 1,464 files fully pass (~43%).** The stricter all-or-nothing
