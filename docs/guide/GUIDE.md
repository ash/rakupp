...
- **Per-test — ~90% of all declared tests pass (197,053 / ~218,589).** This is the
  ...
  (197,053 / 203,478) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~25k tests in aborting files.
- **Coverage — 593 / 1,462 files fully pass (~41%).** The stricter all-or-nothing
