...
| `numeric-op-on-string` | warning | Arithmetic (`+` `-` `*` `/` `%` `**`) with a string literal that is not a number, e.g. `"a" + 1`. Unlike the comparison above this does not answer anything: the conversion fails and the program **dies**, possibly on a branch no test takes. `+` meaning `~` is the usual cause. Empty and whitespace-only strings numify to 0 without complaint (`"" + 1` is 1) and are not flagged. |
| `assignment-in-condition` | warning | An `if`/`unless` whose condition assigns to an already-declared variable — `if $x = 5` — which stores 5 and tests it, where `==` was almost certainly meant. `if my $x = f()` is a binding and is never flagged, nor is any `while` condition, where assigning is idiomatic. |
| `duplicate-condition` | warning | The same condition twice in one `if`/`elsif` chain, so the later branch can never be taken. Compares only conditions built from plain variables, literals and comparisons between them: anything containing a call is left alone, because two calls need not answer the same thing. |
