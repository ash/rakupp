...
Roast standing: measured per individual test, **~91% of all declared tests pass** (199,980 / ~219,294, counting tests in files that abort before running); on the stricter file bar, **660 / 1,464 fully pass (~45%)** (672 partial, 117 no-TAP, 15 timeout). See [COUNTING.md](../status/COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
