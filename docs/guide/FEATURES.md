...
- A negative list index does not count from the end — `@a[*-1]` is the from-the-end form. Written literally it is refused at compile time (`@a[-1]`, `@a[0..-1]` → `X::Obsolete`, naming `*-1` as the replacement); reached indirectly (`my $i = -1; @a[$i]`) it is out of range — a Failure, no Python-style wraparound — and an unhandled Failure propagates through further indexing (`@a[$i][0]`). `.AT-POS`/`.ASSIGN-POS`/`.DELETE-POS` throw `X::OutOfRange` on one; `.EXISTS-POS` simply answers `False`
...
- `IO::Path` tests & stat: `.e .f .d .r .w .x` (real `access()`), `.s` (size), `.z` (empty), `.l` (symlink), `.mode` (an `IntStr` — `0o644` numerically, `"0644"` as a string), `.user`/`.group` (the owner's numeric ids), `.modified/.changed/.accessed` (distinct-precision Instants), `.chmod`; filetest-adverb smartmatch `$path.IO ~~ :e/:d/:f/:r/:w/:x/:s/:z/:l`; `chmod`/`unlink`/`mkdir` subs
...
- `$*CWD $*EXECUTABLE $*ARGS $*RAKU/$*PERL` (`.compiler.name` = "Raku++", backend "cpp"; `.compiler.version` deliberately reports the **Rakudo era tracked**, `v2026.08`, since modules gate features on it — the Raku++ release is `.release`/`.id`), `$*DISTRO $*KERNEL $*VM` (`$*VM.name` is `cpp` — this engine's backends are `cpp` and `js`, as Rakudo's are `moar`/`jvm`/`js`; a build hook run by `rakupp install` sees `moar` instead, because the ecosystem's build recipes have no other branch, and `RAKUPP_VM_NAME` asserts that dialect for a whole run) `$*THREAD $*SCHEDULER`
