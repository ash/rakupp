...
Roast standing: measured per individual test, **~90% of all declared tests pass** (198,956 / ~218,764, counting tests in files that abort before running); on the stricter file bar, **642 / 1,464 fully pass (~44%)** (686 partial, 121 no-TAP, 15 timeout). See [COUNTING.md](../status/COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
