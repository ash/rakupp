...
Roast standing: measured per individual test, **~90% of all declared tests pass** (197,060 / ~217,110, counting tests in files that abort before running); on the stricter file bar, **633 / 1,462 fully pass (~43%)** (675 partial, 143 no-TAP, 11 timeout). See [COUNTING.md](../status/COUNTING.md) for how these are defined. (The declared denominator grows as parse fixes land: files that previously died before announcing a plan now declare their real, often larger, dynamic plans.)
