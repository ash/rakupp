...
dispatch stub Rakudo writes, runnable by name once the store's `bin/` is on
`PATH`. The wrapper's shebang names `rakupp`, so a command `rakupp install`
put there runs under rakupp even on a machine where `raku` is Rakudo. When
nothing on `PATH` answers to `rakupp` (a `build/rakupp` run by its path), the
install links `~/.raku/bin/rakupp` to this engine and says so, so the one
`PATH` entry above also resolves the shebangs. `uninstall` removes the
wrapper unless another installed dist still provides a script of that name.
(Installs from before this existed have no wrappers, and older wrappers
name `raku`; a `reinstall` of the dist writes them anew.)
