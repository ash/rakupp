...
full machine-extracted inventories (171 subroutines, 485 methods) are in the
...
The 171 subroutines registered by `Interpreter::registerBuiltins()`
(`src/Builtins.cpp`), alphabetically:

```
!!! ... ??? EVAL Slip VAR __format__ __radix abs acosec acotan append asec atan2
await bag bail-out bail_out cache callsame callwith ceiling chars chdir chmod
chr chrs cis classify close cmp-ok cosec cotan dd defined diag die dies-ok dir
does-ok done done-testing done_testing eager elems emit end eval-dies-ok
eval-lives-ok exit exp expmod fail fails-like first flat floor flunk get getc
grep hash index is is-approx is-deeply is-prime isa-ok isnt item join keys kv
lastcall lazy lc like lines list lives-ok log log10 log2 make make-temp-dir
make-temp-file map max min minmax mix mkdir nextsame nextwith nok not note ok
open ord ords pass plan pop prepend print printf proceed produce prompt push put
quietly react reduce reverse rindex rmdir round roundrobin run samewith say sec
set shell shift sign sink skip skip-rest sleep sleep-till sleep-timer slip slurp
snip so sort splice split sprintf spurt sqrt srand start subtest succeed sum
supply take tc throws-like times todo uc unimatch uniname uninames uniprop
unival univals unlike unlink unshift use-ok values warn whenever words zip
```
...
The 485 method names dispatched in `Interpreter::methodCall()`
...
```
. .. ASSIGN-KEY ASSIGN-POS AT-KEY AT-POS Array BIND-POS Bag BagHash Bool Channel
Complex DEFINITE DELETE-KEY DELETE-POS DISTROnames Date EVAL EXISTS-KEY
EXISTS-POS Failure FatRat HOW Hash IO Int KERNELnames List Map Mix MixHash NFC
NFD NFKC NFKD Num Numeric Range Rat Real Seq Set SetHash Slip Str Stringy Supply
UInt VAR VMnames WHAT WHICH WHO ^name abs absolute accept accessed acos acosec
acosh acot acotan acquire acsc act add add_method after all allocate allof
antipair antipairs any anyof api-matcher append archname are arg arity asec asin
asinh assuming ast at atan atan2 atanh attributes auth auth-matcher authority
backend base base-repeating basename batch before bits bless bool-only break
broken bytes cache can candidates canonpath caps categorize categorize-list
...
cosh cot cotan count count-only cpu-arch cpu-cores created csc d day
day-fraction day-of-week day-of-year daycount days-in-month decode default
defined delayed denominator desc dir dirname do does done dow e eager earlier
elems emit empty encode end endian ends-with enums eof err exception exists
exitcode exp expmod extension f fail fc find_method finish first first-index
flat flatmap flip floor fmt from from-list from-posix get getc getline gist grab
grep grep-index handled has_accessor hash head hour hyper id im in in-timezone
index indices infinite interval invert is-absolute is-deterministic
is-initial-thread is-lazy is-leap-year is-monotonically-increasing is-prime
is-relative is-win isNaN is_dispatcher iterator join julian-date keep kept key
keyof keys kill kv kxxv l lang-version last-date-in-month later lazy lc level
lines list live local lock log lookup made magnitude map match max maxpairs
merge message methods migrate min minmax minpairs minute mkdir mode modified
modified-julian-date month mro multi name narrow new nl-in nl-out none norm not
note now nude numerator of offset on-demand one open ord ords orig out pair
pairs parent parents parse parse-base parsefile parts path path-sep perl
permutations pick pid plus polar poll polymod pop posix postmatch pred prematch
prepend preserving print printf produce protect protect-or-queue-on-recursion
pull-one push push-all push-at-least push-exactly push-until-lazy put quit r
race raku rand re read read-bits read-ubits readchars readonly reals receive
recv reduce release repeated resolve result resume rethrow returns reverse
rindex rmdir role_typecheck_list roles roll rotate rotor round run rw s samecase
say schedule-on sec second self send shift short-name sibling sign signal
...
squish stable start starts-with status stderr stdout subparse subst subst-mutate
substr substr-eq substr-rw succ sum tail take tan tanh tap tc tclc then throw
timezone to today toggle total trans trim trim-leading trim-trailing truncate
truncated-to try-acquire try_acquire type uc unimatch uniname uninames uniprop
unique unival univals unlink unlock unpolar unshift unwrap utc value values
version version-matcher vow w wait what whatever whole-second wordcase words
wrap write write-bits write-ubits x year yyyy-mm-dd z zip
```
