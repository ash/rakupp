...
std::string transpileToCpp(Program& prog, bool optimize, const std::string& srcPath,
    ...
    // main()
    g.out << "namespace rakupp { int rakuppMainOnBigStack(int (*)(void*), void*); void setupConsole();\n"
             ...
             // srcFile_ names the .raku this was built from ($?FILE, "at FILE line N");
             // the PROGRAM is this binary, named by the command that ran it, which is
             // what the usage text and $*PROGRAM-NAME have to say. $*EXECUTABLE is the
             // same file resolved: a compiled binary is its own interpreter.
             "    RT.srcFile_ = " + cesc(srcPath) + ";\n"
             "    RT.progName_ = argc > 0 ? argv[0] : " + cesc(srcPath) + ";\n"
             "    { std::string __e = RT.progName_; char __rp[4096];\n"
             "      if (RAKUPP_REALPATH(__e.c_str(), __rp)) __e = __rp; RT.execPath_ = __e; }\n"
             "    __rakupp_register();\n"
    ...
    g.out << "    } catch (const ExitEx& e) { std::cout.flush(); return e.code; }\n"
             ...
             "    rakupp::setupConsole();  // Windows: UTF-8 + VT console (no-op elsewhere)\n"
