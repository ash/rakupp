...
// ---------------------------------------------------------------------------
// Every Jupyter message carries an HMAC-SHA256 signature of its four JSON
// blobs, keyed by the connection file's `key`: a kernel that cannot verify one
// has to reject the message, so this cannot be optional. The implementation is
// src/Digest.cpp, shared with the `digest` tag's primitives — it lived here,
// file-local, until DATA-PLAN P3 lifted it out. tools/jupyter-smoke.raku still
// gates it against RFC 4231 vectors, and now gates the primitives too.
// ---------------------------------------------------------------------------
...
std::string hmacSha256Hex(const std::string& key, const std::vector<std::string>& parts) {
    std::string msg;
    for (auto& p : parts) msg += p;
    return digestToHex(digestHmac(DigestAlgo::SHA256, key, msg));
}
