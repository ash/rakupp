...
**Headline: ~76% of all declared Roast tests pass** (143,490 / 188,486); on the
stricter file bar, ~27% of files fully pass (393 / 1,464). The per-file breakdown
...
|---|---:|---:|
| **Fully passing** | **393** | **25%** |
| Partially passing | 608 | 42% |
...
**143,490 of ~188,486 declared tests — ~76%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 143,490 / 148,775 (~93%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 143,490 / 160,950 (~86%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 143,490 / 188,486 (~76%) | + tests in parse-error files, recovered from source |
...
_Snapshot: 393 / 1,464 files fully passing (~27% coverage); 608 partial,
453 no-TAP, 10 timeout. Reached-assertion pass rate 143,490 / 148,775 (see
