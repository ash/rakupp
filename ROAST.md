...
**Headline: ~57% of all declared Roast tests pass** (132,646 / 230,851); on the
stricter file bar, ~22% of files fully pass (322 / 1,464). The per-file breakdown
...
**132,646 of ~230,851 declared tests — ~57%**. "Declared" means every test the
...
|---|---|---|
| tests that **ran** | 132,646 / 189,362 (~70%) | only assertions files actually emitted — flatters, ignores aborts |
| tests **planned** (files that emitted a plan) | 132,646 / 200,724 (~66%) | + tests lost when a file aborts mid-plan |
| **all declared** tests | 132,646 / 230,851 (~57%) | + tests in parse-error files, recovered from source |
...
_Snapshot: 322 / 1,464 files fully passing (~22% coverage); 608 partial,
524 no-TAP, 3 timeout. Reached-assertion pass rate 132,646 / 189,362 (see
