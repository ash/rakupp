...
# that the size budgets hold (-all ≤ 5.5 MB, bare --slim ≤ 6.0 MB on hello) —
...

{
    ...
    $all-size = $bin.IO.s;
    check $*KERNEL.name ne 'darwin' || $all-size <= 5.5 * 1024 * 1024,
          "--slim=-all hello is within the 5.5 MB darwin budget ($all-size bytes; darwin-only gate)";
