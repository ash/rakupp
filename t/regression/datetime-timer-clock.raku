...
my $fail = 0;
# The pass line must be BYTE-IDENTICAL run to run: tools/slim-diff.raku compares
# this program's output between a full and a --slim build, and a measured
# duration interpolated into an `ok` line differs by a millisecond and reads as
# a cut that changed behaviour. Measurements are shown only when a check FAILS.
sub check($ok, $what, $detail = '') { $fail++ unless $ok; say ($ok ?? "ok   " !! "FAIL ") ~ $what ~ ($ok || !$detail ?? '' !! " — $detail") }
