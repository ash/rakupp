...
#
# "Native" has to be CHECKED, not hoped for. Taking the first candidate that
# exists is the same trap perf-guard and run-optbench both had: on the machine of
# record `build/` holds an x86_64 build beside a native `build-arm64/`, and this
# loop took it. At the v3.22.0 release that binary was ALSO two releases stale
# (v3.20.1, 2026-08-27), so the playground's examples.js — the code every visitor
# sees in /play/ — was generated by an engine two tags behind the one the page
# announces. Nothing failed; it just quietly used the wrong compiler.
host_arch() {
  if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    [[ "$(sysctl -n hw.optional.arm64 2>/dev/null)" == "1" ]] && { echo arm64; return; }
  fi
  uname -m
}
binary_arch() {
...
if [[ -z "$RAKUPP_BIN" ]]; then
  ...
  # warning below still says something useful
  for c in "$ROOT_DIR/build/rakupp" "$ROOT_DIR/build-arm64/rakupp" "$ROOT_DIR/rakupp"; do
    [[ -x "$c" ]] || continue
    if [[ "$(binary_arch "$c")" == "$HOST_ARCH" ]]; then RAKUPP_BIN="$c"; break; fi
    [[ -z "$RAKUPP_BIN" ]] && RAKUPP_BIN="$c"
  done
fi
