...
- **Per-test — ~81% of all declared tests pass (152,316 / ~188,486).** This is the
  ...
  (152,316 / 157,059) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 401 / 1,464 files fully pass (~27%).** The stricter all-or-nothing
