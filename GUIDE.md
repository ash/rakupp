...
- **Per-test — ~57% of all declared tests pass (132,692 / ~230,851).** This is the
  ...
  (132,692 / 189,362) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 323 / 1,464 files fully pass (~22%).** The stricter all-or-nothing
