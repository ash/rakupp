...
- **Per-test — ~76% of all declared tests pass (143,490 / ~188,486).** This is the
  ...
  (143,490 / 148,775) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 393 / 1,464 files fully pass (~27%).** The stricter all-or-nothing
