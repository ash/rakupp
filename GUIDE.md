...
- **Per-test — ~76% of all declared tests pass (143,744 / ~188,835).** This is the
  ...
  (143,744 / 149,126) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 390 / 1,464 files fully pass (~27%).** The stricter all-or-nothing
