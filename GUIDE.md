...
- **Per-test — ~57% of all declared tests pass (132,646 / ~230,851).** This is the
  ...
  (132,646 / 189,362) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 322 / 1,464 files fully pass (~22%).** The stricter all-or-nothing
