...
- **Per-test — ~76% of all declared tests pass (143,537 / ~188,486).** This is the
  ...
  (143,537 / 148,788) — that variant counts only assertions in files that produce
  TAP, so it flatters by ignoring the ~31k tests in aborting files.
- **Coverage — 395 / 1,464 files fully pass (~27%).** The stricter all-or-nothing
